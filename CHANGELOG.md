...
### Added
- `pyMyriad.clinical.summary_table(df, *, variables, arm_col, subject_col, by=None, stats=("n", "mean_sd", "median_iqr", "min_max"), as_gt=False, title=None, subtitle=None)` — the demographics / baseline-characteristics ("Table 1") counterpart to `lab_summary_table()`: columns are treatment Arm; rows are the variables listed in `variables` (an explicit `{column: "continuous"|"categorical"}` mapping, not inferred from dtype), each summarized per its declared type — continuous variables get one row per `stats` entry (reusing `lab_summary_table`'s existing statistic formatters), categorical variables get one row per observed category level formatted as `"n (pct%)"`, where the percentage denominator is the count of unique `subject_col` values in that level's Arm total (#76). The denominator reuses `AnalysisTree`'s existing `denom=`/`_N` mechanism (`denom=subject_col`) — the same primitive already used elsewhere in the library for proportions — rather than a bespoke lookup, so `subject_col` is a required argument and percentages stay correct even if `df` has more than one row per subject. An optional `by=` column stratifies rows into blocks (e.g. one demographics table per sex), rendered as bold row-group headers via `great_tables`' `tab_stub(groupname_col=...)` when `as_gt=True`. Because continuous and categorical variables don't naturally share one `AnalysisTree`, each variable gets its own small tree + `simple_table(..., by=arm_col, pivot_statistics=False)` call, and the per-variable blocks are vertically concatenated and re-sorted into `variables` dict order. A category level absent from one arm renders as `NaN`, consistent with `lab_summary_table`'s existing missing-cell behavior.
