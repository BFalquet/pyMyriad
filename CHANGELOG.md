...
### Added
- `AGENTS.md` at the repo root and `.agents/skills/` folder with reusable agent skill files (`test`, `pr-workflow`, `add-analysis`, `perform-analysis`) to provide a single source of truth for AI coding assistants (#85, #88).
- `AnalysisTree.multi_simple_analysis(var, continuous_fun=("n", "mean_sd", "median_iqr", "min_max"), categorical_fun=("n (pct)",))` (also on `SplitNode`) — adds one sibling branch per variable in `var` (an explicit `{column: "continuous"|"categorical"}` mapping), dispatched to predefined functions: continuous variables get a single `AnalysisNode` computing each `continuous_fun` entry; categorical variables get a `SplitNode` (one child per observed level) computing each `categorical_fun` entry (#76). This is a direct tree-builder, not a post-hoc table reshape — the resulting tree literally shows one labeled branch per variable, since a plain `analyze_by()` can't vary its row count per variable and, once a terminal `analyze_by()` node exists at a branch, `split_by()` can no longer add a sibling split there. Predefined functions live in `pyMyriad.analysis_tree.CONTINUOUS_SIMPLE_FUNCTIONS` (`n`, `mean`, `sd`, `median`, `q1`, `q3`, `min`, `max`, plus formatted combos `mean_sd`/`median_iqr`/`min_max`) and `CATEGORICAL_SIMPLE_FUNCTIONS` (`n`, `n (pct)`, `pct` — the latter two require `denom` set on the tree, reusing the existing `denom=`/`_N` mechanism for percentage denominators rather than a bespoke lookup).
- `pyMyriad.clinical.summary_table(df, *, variables, arm_col, subject_col, by=None, stats=("n", "mean_sd", "median_iqr", "min_max"), as_gt=False, title=None, subtitle=None)` — the demographics / baseline-characteristics ("Table 1") counterpart to `lab_summary_table()`: columns are treatment Arm; rows are the variables listed in `variables`, each summarized per its declared type via `AnalysisTree.multi_simple_analysis()` — continuous variables get one row per `stats` entry, categorical variables get one row per observed category level formatted as `"n (pct%)"` (percentage denominator = unique `subject_col` count in that level's Arm total) (#76). Internally builds a *single* tree (split by `by` if given, then `arm_col`, then `multi_simple_analysis()`) and flattens it with one `simple_table(..., include_label=True)` call — no per-variable tree, no manual `pd.concat`. Returned columns are `(By?), Variable, Level, Statistic, {arm}...`; `Level` is blank for continuous rows. An optional `by=` column stratifies rows into blocks (e.g. one demographics table per sex), rendered as bold row-group headers via `great_tables`' `tab_stub(groupname_col=...)` when `as_gt=True`. A category level absent from one arm renders as `NaN`.
- `pyMyriad.clinical` module with `lab_summary_table(df, *, value_col, visit_col, arm_col, subject_col=None, baseline_level=None, stats=("n", "mean_sd", "median_iqr", "min_max"), change_col="CHG", as_gt=False, title=None, subtitle=None)` — produces the canonical clinical-trial lab table directly from subject-level long-format data: rows are Visit, sub-divided into one row per requested descriptive statistic (n / Mean (SD) / Median (Q1, Q3) / Min-Max); columns are treatment Arm, sub-divided into observed Value and Change from Baseline (#65). If `df` already has the `change_col` column it is used as-is (assumed pre-computed); otherwise it is computed with `change_from_baseline()`, which is when `subject_col`/`baseline_level` are required. Builds the table with two `analyze_by(label=...)` calls (Value/Change) and a single `simple_table(..., by=["Arm", "Analysis"], pivot_statistics=False)` pivot. Visit/Arm ordering follows the input's categorical dtype where present. Pass `as_gt=True` to get a formatted `great_tables.GT` with Arm spanners instead of a plain DataFrame.
