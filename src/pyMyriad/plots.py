...
def forest_plot(
    ...
    res["pivot_lvl"] = res["pivot_lvl"].apply(
        lambda x: ".none" if x is None else " >> ".join(x)
    )
    res["pivot_split"] = res["pivot_split"].apply(
        lambda x: ".none" if x is None else " >> ".join(x)
    )
    res["label"] = res.apply(
        lambda row: (
            "__not__analysis__" if (row["type"] != "analysis") else row["label"]
        ),
        axis=1,
    )
    res["label"] = res["label"].apply(
        lambda x: "Unlabelled" if len(x) == 0 else x
    )  # 0 len label make pivot delete the corresponding lines.

    # Remove the last element from path_pivot for y_label
    res["y_label"] = res.apply(
        lambda row: (
            str(row["path_pivot"][-2])
            if row["type"] == "analysis"
            else row["lvl"] or row["split"] or "root"
        ),
        axis=1,
    )

    # Remove unnecessary rows
    available_analysis = res.loc[res["type"] == "analysis", "path_pivot"].apply(
        lambda x: x[:-1]
    )
    available_analysis = available_analysis.apply(lambda x: " >> ".join(x)).unique()
    res["path_pivot"] = res["path_pivot"].apply(lambda x: " >> ".join(x))
    res = res.loc[
        ~(res["path_pivot"].isin(available_analysis) & (res["type"] != "analysis"))
    ]
    ...
    rank_dict = {
        p: -i
        for i, p in enumerate(res.loc[~res["path_pivot"].duplicated(), "path_pivot"])
    }
    res["y"] = res["path_pivot"].map(rank_dict)

    res["y_label"] = res.apply(
        lambda row: (" " * row["depth"] * 2) + row["y_label"], axis=1
    )
    ...
    if (len(res_data["pivot_lvl"].unique()) > 1) & jitter:
        ...
        norm_codes = (codes - np.min(codes)) / (
            np.max(codes) - np.min(codes)
        ) * 0.4 - 0.2
    ...
    def set_ticks(data, **kwargs):
        ...
        ax.tick_params(
            axis="y", which="major", labelsize="medium", pad=max_label_len * 5
        )
...
def plot_1d(data, type: str = "forest", x: str = "x", x_err: str = "err", **kwargs):
    ...
    if type == "forest":
        plt.errorbar(data[x], data[y], xerr=data[x_err], marker="o", linestyle="")

    elif type == "range":
        plt.errorbar(data[x], data[y], xerr=data[x_err], marker="", linestyle="")

    elif type == "point":
        plt.scatter(data[x], data[y], marker="o")

    elif type == "bar":
        plt.barh(y=data[y], width=data[x], height=0.4)
...
def plot_distribution(data, type: str = "scatter", x: str = None, **kwargs):
    ...
    if isinstance(x, str):
        assert x in x_series.iloc[0].columns, (
            f"{x} not found in the data, available columns: {x_series.iloc[0].columns.tolist()}"
        )
        ...
        if len(close_matches) == 0:
            raise ValueError(
                f"Could not find a column name similar to the facet name: {current_facet}. Please provide an explicit x mapping."
            )
        ...
        warnings.warn(
            f"Warning: x parameter is None. Using column '{x_col}' for facet '{current_facet}'."
        )
    else:
        raise ValueError(
            "x must be a string or a dictionary mapping facet labels to column names."
        )

    y_series = data[[y, "y_label", "pivot_lvl"]]
    merge_data = pd.merge(
        y_series, x_series, left_index=True, right_index=True, how="left"
    )

    if type == "scatter":
        plt.scatter(merge_data["summary"], merge_data[y], marker="|", linestyle="")
        plt.xlabel(x_label)
        ...
        for i, y_pos in enumerate(unique_y):
            ...
            plt.plot(
                [q1, q3], [y_pos, y_pos], color="k", linewidth=4, alpha=0.2
            )  # IQR line
