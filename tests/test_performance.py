...
def test_complex_tree_small_dataset(small_dataset):
    ...
    tree = (
        ...
        .analyze_by(
            ...
            std_income=lambda df: np.std(df.Income),
            count=lambda df: len(df),
            savings_rate=lambda df: (
                np.mean(df.Savings / df.Income) if len(df) > 0 else 0
            ),
        )
    )
...
def test_performance_summary(small_dataset, medium_dataset, large_dataset):
    ...
    for tree_type, dataset_size, duration, memory in results:
        print(
            f"{tree_type:<25} {dataset_size:<10} {format_duration(duration):<15} {format_memory(memory):<15}"
        )

    print(f"{'=' * 80}\n")

    # Basic assertions
    assert all(dur < 30.0 for _, _, dur, _ in results), (
